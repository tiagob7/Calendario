// js/gerir-calendarios.js — Lógica específica de gerir-calendarios.html
// Requer: js/shared.js
...
async function publicarFeriados() {
  ...
  if (!escritorios.length) { alert('Seleciona pelo menos um escritório.'); return; }
  ...
  if (!selecionados.length) { alert('Seleciona pelo menos um feriado.'); return; }
...
function adicionarEvento() {
  ...
  if (!diaIni || diaIni < 1 || diaIni > 31) { alert('Dia de início inválido (1–31).'); return; }
  if (diaFim < diaIni || diaFim > 31) { alert('Dia de fim inválido.'); return; }
  if (!label) { alert('Introduz uma descrição para o evento.'); return; }
...
async function publicarEventos() {
  if (!eventosPendentes.length) { alert('Adiciona pelo menos um evento antes de publicar.'); return; }
...
async function apagarEventos() {
  ...
  if (!escritorios.length) { alert('Seleciona pelo menos um escritório.'); return; }
  if (modo === 'texto' && !textoFiltro) { alert('Introduz o texto a pesquisar.'); return; }
