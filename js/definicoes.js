...
async function guardarEscritorio() {
  ...
  if (!nome) { toast('Introduz o nome do escritório.'); return; }
  if (!id || !/^[a-z0-9]+$/.test(id)) { toast('O ID só pode ter letras minúsculas e números, sem espaços.'); return; }
  ...
  try {
    if (escEditandoId) {
      ...
      if (escritoriosData.find(e => e.id === id)) {
        toast('Já existe um escritório com esse ID.'); btn.disabled=false; btn.textContent='Guardar'; return;
      }
