// js/dashboard.js — Lógica específica de dashboard.html
// Requer: js/shared.js
...
function startSync(escritorio) {
  ...
  const unsubCom = db.collection('comunicados')
    ...
    .onSnapshot(snap => {
      ...
      checkAllLoaded();
    }, err => { console.error('com:', err); loadedFlags.com = true; checkAllLoaded(); });
  ...
  const unsubAdm = db.collection('admissoes')
    ...
    .onSnapshot(snap => {
      ...
      checkAllLoaded();
    }, err => { console.error('adm:', err); loadedFlags.adm = true; checkAllLoaded(); });
  ...
  window._dashCalUnsub = db.collection('calendarios').doc(calDocId).onSnapshot(snap => {
    ...
    checkAllLoaded();
  }, err => { console.error('cal:', err); loadedFlags.cal = true; checkAllLoaded(); });
  ...
  const unsubRec = db.collection('reclamacoes_horas')
    ...
    }, err => { console.error('rec:', err); loadedFlags.rec = true; checkAllLoaded(); });
