// js/reclamacoes.js — Lógica específica de reclamacoes.html
// Requer: js/shared.js
...
async function updateEstado(id,novoEstado,nota){
  ...
  try{
    ...
    const hist=snap.data().historico||[];
...
async function deleteRec(id){
  if(!confirm('Eliminar esta reclamação?'))return;
  expandedIds.delete(id);
  try{await col.doc(id).delete();toast('Eliminada.');}catch(e){toast('Erro.');}
}

// ── HELPERS ──
function setStatus(msg,color){const el=document.getElementById('syncStatus');if(el){el.textContent=msg;el.style.color=color||'var(--muted)';}}
